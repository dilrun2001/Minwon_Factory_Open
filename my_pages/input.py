...
def show_input():
    if st.session_state.log_in:
        ...
        if st.session_state.answer_format == "None":
...
감사합니다."""
        elif st.session_state.answer_format == "양식 1":
            st.session_state.answer = f"{run_query("SELECT * FROM userdata WHERE id = %s", (st.session_state.id)).iloc[0]['양식']}"
            #st.rerun()
        ...
        st.session_state.urgency = st.sidebar.select_slider(
        ...
        def genereate_response():
            if minwon and answer_format and answer and st.session_state.name:
                    ...
                    st.text_area("답변 결과", value = response, height = 200)
