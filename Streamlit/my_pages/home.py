...
def sidebar_set():
    if st.session_state.file_check:
        ...
        with st.sidebar.expander("데이터 초기화", icon = ":material/clear_all:", expanded = False):
                ...
                with center:
                    st.button("세션 초기화", on_click = minwon_clear, key = "clear")
...
def input_set():
    ...
    if st.session_state['btn_show']:
        ...
        ul, us, ur = st.columns ((1.4, 11.6, 1.4), border = True)
        with ul:
            st.button("이전 단계", key = "input_before_button", on_click=page_convert)
        with ur:
            st.button("다음 단계", key = "input_after_button", on_click = page_convert)
...
def show_home():
    ...
    st.markdown('''
    ##### 본 페이지는 지난 5월 14일, 사하구청 관계자와의 면담 이후 시스템 방향성이 대폭 수정된 버전입니다.      
    ''')
    ...
    if st.session_state['btn_show']:
        ...
        ul, us, ur = st.columns((1.4, 11.2, 1.4), border = True)
        with ur:
            st.button("다음 단계", key = "input_page_show", on_click  = page_convert)
...
def show_select():
    ...
    if st.session_state['btn_show']:
        ...
        ul, us, ur = st.columns ((1.4, 11.6, 1.4), border = True)
        with ul:
            st.button("이전 단계", key = "select_before_button", on_click=page_convert)
        with ur:
            st.button("다음 단계", key = "select_after_button", on_click = page_convert)
...
def show_input():
    ...
    if st.session_state['btn_show']:
        ul, us, ur = st.columns ((1.4, 11.5, 1.4), border = True)
        with ul:
            st.button("이전 단계", key = "input_before_button", on_click=page_convert)
        with ur:
            st.button("다음 단계", key = "input_after_button", on_click = page_convert)
...
def show_result():
    ...
    st.text_area("답변 결과", value = st.session_state.answer, height = 200, key="result")
    ul, us, ur = st.columns ((1.4, 11.6, 1.4), border = True)
    with ul:
        st.button("이전 단계", key = "result_before_button", on_click=page_convert)
    with ur:
        st.button("처음 화면으로", key = "result_after_button", on_click = page_convert)
